...
from .io_operetions import quality_export, quality_export_mpi


def grid_quality_check(gdcurv: 'GridData', cfgs: dict,
                       export_mode: str = 'single') -> None:
    ...
    def _export(var, quality_name):
        if export_mode == 'mpi':
            quality_export_mpi(var, gdcurv, cfgs, quality_name)
        else:
            quality_export(gdcurv, var, cfgs['grid_export_dir'], quality_name)
...
def cal_orth(var: np.ndarray, x2d: np.ndarray, z2d: np.ndarray,
    ...
    cos_angle = np.divide(dot, len_xi * len_zt,
                          out=np.zeros_like(dot),
                          where=(len_xi * len_zt) != 0)
...
def cal_smooth_x(var: np.ndarray, x2d: np.ndarray, z2d: np.ndarray,
                 ni: int):
    x_xi1 = x2d[:, 1:ni-1] - x2d[:, 0:ni-2]
    z_xi1 = z2d[:, 1:ni-1] - z2d[:, 0:ni-2]

    x_xi2 = x2d[:, 2:ni] - x2d[:, 1:ni-1]
    z_xi2 = z2d[:, 2:ni] - z2d[:, 1:ni-1]
...
def cal_smooth_z(var: np.ndarray, x2d: np.ndarray, z2d: np.ndarray,
                 nk: int):
    x_zt1 = x2d[1:nk-1, :] - x2d[0:nk-2, :]
    z_zt1 = z2d[1:nk-1, :] - z2d[0:nk-2, :]

    x_zt2 = x2d[2:nk, :] - x2d[1:nk-1, :]
    z_zt2 = z2d[2:nk, :] - z2d[1:nk-1, :]
...
def cal_min_dist(gdcurv: 'GridData') -> Tuple[int, int, float]:
    """Calculate minimum point-to-line distance for CFL stability."""
    x2d = gdcurv.x2d
    z2d = gdcurv.z2d
    ni = gdcurv.ni
    nk = gdcurv.nk
