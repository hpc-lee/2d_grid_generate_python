...
This package provides Python implementations of three grid generation methods, documented in a doctoral thesis~[1]:
...
\begin{itemize}
    ...
    \item Computational coordinates: $(\xi, \zeta)$, where $\xi$ corresponds to the horizontal (x) direction and $\zeta$ corresponds to the vertical (z) direction
    \item In the code, $\xi$ is referred to as \texttt{xi} and $\zeta$ as \texttt{zt}
    \item Grid indices: $i \in [0, N_x-1]$ for $\xi$, $k \in [0, N_z-1]$ for $\zeta$
\end{itemize}
...
\begin{aligned}
    ...
    g_{12} &= \rvec_{,\xi} \cdot \rvec_{,\zeta} = x_{,\xi} x_{,\zeta} + z_{,\xi} z_{,\zeta}, \\
    g_{22} &= \rvec_{,\zeta} \cdot \rvec_{,\zeta} = x_{,\zeta}^2 + z_{,\zeta}^2.
\end{aligned}
\end{equation}
...
\begin{equation}
    J = \begin{vmatrix} x_{,\xi} & x_{,\zeta} \\ z_{,\xi} & z_{,\zeta} \end{vmatrix} = x_{,\xi} z_{,\zeta} - z_{,\xi} x_{,\zeta}.
\end{equation}
...
\begin{aligned}
    ...
    x_{,\zeta} &\approx \frac{1}{2}(x_{i,k+1} - x_{i,k-1}).
\end{aligned}
\end{equation}
...
\begin{aligned}
    ...
    x_{,\zeta\zeta} &\approx x_{i,k-1} - 2x_{i,k} + x_{i,k+1}.
\end{aligned}
\end{equation}

Mixed derivative:
\begin{equation}
    x_{,\xi\zeta} \approx \frac{1}{4}(x_{i+1,k+1} + x_{i-1,k-1} - x_{i-1,k+1} - x_{i+1,k-1}).
\end{equation}
...
\begin{equation}
    g_{22} f_{,\xi\xi} + g_{11} f_{,\zeta\zeta} - 2g_{12} f_{,\xi\zeta} = 0,
    \label{eq:para-gov}
\end{equation}
...
\begin{equation}
    \varepsilon_s = e^{-a\zeta}, \quad \zeta = \frac{j-1}{J-1}.
\end{equation}
...
\begin{equation}
    g_{22}(f_{i-1,j} - 2f_{i,j} + f_{i+1,j}) + g_{11}(f_{i,j-1} - 2f_{i,j} + f_{i,j+1}) - 2g_{12} f_{,\xi\zeta} = 0.
\end{equation}
...
\begin{aligned}
    ...
    b_i &= -2(g_{11} + g_{22}), \\
    c_i &= g_{22}, \\
    D_i &= -g_{11}(f_{i,j-1} + f_{i,j+1}) + 2g_{12} f_{,\xi\zeta}.
\end{aligned}
\end{equation}
...
\begin{equation}
    \rvec_{j,\xi} \cdot \rvec_{j,\zeta} = 0.
    \label{eq:hyper-orth}
\end{equation}

\textbf{Area conservation condition}:
\begin{equation}
    \rvec_{j,\xi} \times \rvec_{j,\zeta} = \Delta A_j,
    \label{eq:hyper-area}
\end{equation}
...
\begin{equation}
    \bm{A}_{j-1} \rvec_{j,\xi} + \bm{B}_{j-1} \rvec_{j,\zeta} = \bm{f}_j,
\end{equation}
where the coefficient matrices are:
\begin{equation}
    \bm{A} = \begin{bmatrix} x_{j-1,\zeta} & z_{j-1,\zeta} \\ z_{j-1,\zeta} & -x_{j-1,\zeta} \end{bmatrix}, \quad
...
The $\zeta$-direction derivatives are approximated from the known layer:
\begin{equation}
\begin{aligned}
    x_{j-1,\zeta} &= -\frac{z_{j-1,\xi} \Delta A_{j-1}}{x_{j-1,\xi}^2 + z_{j-1,\xi}^2}, \\
    z_{j-1,\zeta} &= \frac{x_{j-1,\xi} \Delta A_{j-1}}{x_{j-1,\xi}^2 + z_{j-1,\xi}^2}.
\end{aligned}
\end{equation}
...
\begin{equation}
    N_\xi = \sqrt{\frac{x_{,\zeta}^2 + z_{,\zeta}^2}{x_{,\xi}^2 + z_{,\xi}^2}}.
\end{equation}
...
\begin{equation}
    g_{22} \rvec_{,\xi\xi} - 2g_{12} \rvec_{,\xi\zeta} + g_{11} \rvec_{,\zeta\zeta} + g_{22} P \rvec_{,\xi} + g_{11} Q \rvec_{,\zeta} = 0.
    \label{eq:ellip-gov}
\end{equation}
...
The initial grid is generated using linear transfinite interpolation from the four boundaries. Let $\xi = i/(N_x-1)$ and $\zeta = k/(N_z-1)$ denote normalized coordinates.

The TFI formula is:
\begin{equation}
    \rvec(\xi, \zeta) = \bm{U}(\xi, \zeta) + \bm{W}(\xi, \zeta) - \bm{UW}(\xi, \zeta),
\end{equation}
where:

$\xi$-direction interpolation (from x-boundaries):
\begin{equation}
    \bm{U}(\xi, \zeta) = (1-\xi) \bm{B}_{x1}(\zeta) + \xi \bm{B}_{x2}(\zeta),
\end{equation}

$\zeta$-direction interpolation (from z-boundaries):
\begin{equation}
    \bm{W}(\xi, \zeta) = (1-\zeta) \bm{B}_{z1}(\xi) + \zeta \bm{B}_{z2}(\xi),
\end{equation}

Bilinear corner interpolation (to avoid double-counting):
\begin{equation}
\begin{split}
    \bm{UW}(\xi, \zeta) = & (1-\xi)(1-\zeta)\bm{B}_{x1}(0) + (1-\xi)\zeta\bm{B}_{x1}(N_z-1) \\
    & + \xi(1-\zeta)\bm{B}_{x2}(0) + \xi\zeta\bm{B}_{x2}(N_z-1).
\end{split}
\end{equation}
...
\begin{equation}
    \rvec_{k,i}^{\text{tmp}} = \frac{1}{2(g_{11} + g_{22})} \left[ g_{22}(\rvec_{k,i+1} + \rvec_{k,i-1}) + g_{11}(\rvec_{k+1,i} + \rvec_{k-1,i}) - 2g_{12} \rvec_{,\xi\zeta} + g_{22} P \rvec_{,\xi} + g_{11} Q \rvec_{,\zeta} \right],
\end{equation}
\begin{equation}
...
\begin{equation}
\begin{aligned}
    P_{\text{bdry}} &= -\left( \frac{\rvec_{,\xi} \cdot \rvec_{,\xi\xi}}{g_{11}} + \frac{\rvec_{,\xi} \cdot \rvec_{,\zeta\zeta}}{g_{22}} \right), \\
    Q_{\text{bdry}} &= -\left( \frac{\rvec_{,\zeta} \cdot \rvec_{,\xi\xi}}{g_{11}} + \frac{\rvec_{,\zeta} \cdot \rvec_{,\zeta\zeta}}{g_{22}} \right).
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
    P &= w_0 \left[ (1-\xi) P_{x1} + \xi P_{x2} \right] + w_1 \left[ e^{-c_2\zeta} P_{z1} + e^{-c_3(1-\zeta)} P_{z2} \right], \\
    Q &= w_0 \left[ e^{-c_0\xi} Q_{x1} + e^{-c_1(1-\xi)} Q_{x2} \right] + w_1 \left[ (1-\zeta) Q_{z1} + \zeta Q_{z2} \right],
\end{aligned}
\end{equation}
...
\begin{equation}
    Q = 90 - |\theta - 90|,
\end{equation}
where $\theta = \arccos\left( \frac{\rvec_{,\xi} \cdot \rvec_{,\zeta}}{|\rvec_{,\xi}| |\rvec_{,\zeta}|} \right)$.
...
\begin{equation}
    R = \max\left( \frac{|\rvec_{,\xi}|}{|\rvec_{,\zeta}|}, \frac{|\rvec_{,\zeta}|}{|\rvec_{,\xi}|} \right).
\end{equation}
...
\begin{aligned}
    ...
    \Delta s_\zeta &= |\rvec_{i,k+1} - \rvec_{i,k}|.
\end{aligned}
\end{equation}
...
\section{x/z Axis Swap (Reflection)}

When parabolic/hyperbolic grids are generated along the x direction (free surface on a vertical \texttt{bx1/bx2} boundary), the boundary is first reflected into the z direction via a $y=x$ mirror (\texttt{creat\_bz\_bdry.py}) so the marching machinery --- which always marches along $\zeta$ (z) --- can be reused. After generation, the post-processor swaps the grid back to physical coordinates when \texttt{flag\_swap\_xz = 1}:
\begin{equation}
    (x, z)_{\text{phys}} \;=\; (z, x)_{\text{gen}}^{\,T},
\end{equation}
i.e.\ the x and z arrays are exchanged and the array is transposed. The swap is performed immediately after \texttt{read\_import\_coord}, so every subsequent step (sampling, quality checks, export) operates on the physical-coordinate grid. The swapped $N_x$ and $N_z$ (and the corresponding MPI partitioning, PML counts, and sampling factors) are exchanged accordingly.

\section{Automatic Output Configuration}

After processing, \texttt{post\_pro.py} writes a \texttt{config.json} into \texttt{grid\_export\_dir} containing the post-swap grid dimensions and output MPI layout:
\begin{lstlisting}[language=bash]
{
    "number_of_grid_points_x": <post-swap Nx>,
    "number_of_grid_points_z": <post-swap Nz>,
    "number_of_mpiprocs_x": ...,
    "number_of_mpiprocs_z": ...,
    "grid_export_dir": "..."
}
\end{lstlisting}
Downstream tools (e.g.\ \texttt{plotting/draw\_grid.py}) read this file directly, so the user does not need to maintain grid dimensions by hand when a swap changes $N_x \leftrightarrow N_z$.
...
\texttt{execute\_C\_code} & 0/1 & Use C++ kernel acceleration (1) vs.\ pure Python (0) \\
...
\texttt{weight} & [float$\times$2] & Interpolation weights $[w_0, w_1]$ for $\xi$ and $\zeta$ directions \\
...
\texttt{flag\_swap\_xz} & 0/1 & Swap x/z axes (reflect grid back to physical coordinates) \\
...
\begin{itemize}
    ...
    \item g++ $\geq$ 4.9 (C++14, for building the pybind11 kernel)
    \item pybind11 (header-only; include path auto-detected at build time)
\end{itemize}

\section{Building the C++ Kernel}

\begin{lstlisting}[language=bash]
cd src_cpp && bash build.sh
\end{lstlisting}

This compiles \texttt{gridcpp.so}, a pybind11 extension that exchanges arrays with Python through zero-copy \texttt{numpy} buffers at runtime. The build script auto-detects the Python and pybind11 include paths; no CMake is required. The hot kernels (parabolic \texttt{predict\_point}/\texttt{update\_point}, hyperbolic \texttt{cal\_matrix}/\texttt{thomas\_block}, elliptic \texttt{update\_SOR}/TFI) live in this C++ layer, while Python orchestrates the marching and I/O.
